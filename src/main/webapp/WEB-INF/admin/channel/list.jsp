			...
			<table class="table table-striped table-bordered table-advance table-hover">
				<thead>
					<tr>
						...
						<th><i class="icon-briefcase"></i>栏目名称</th>
						<th></th>
						<th class="hidden-phone"><i class="icon-question-sign"></i> 栏目路径</th>
				...
				<tbody>
					<c:forEach var="channel" items="${beans}"> 
					    <tr >
							<td><a href="javascript:display(${channel.channelId})">${channel.channelName}</a></td>
							<td></td>
							...
							<td><c:choose> 
							  ...
							  <c:when test="${channel.display == 1}">   
							   菜单  
							  </c:when> 
							  <c:when test="${channel.display == 2}">   
							    分类  
							  </c:when>  
							  </c:choose>
							  </td> 
						...
						<div class="childChannels" style="display: hidden;">
						 <c:forEach var="child" items="${channel.children}">
				             <tr class="channel${channel.channelId}  hiddendiv">
								<td></td>
								<td><a href="#">${child.channelName}</a></td>
								<td class="hidden-phone">${child.channelPath}</td>
								<td>${child.link}</td>
								<td><c:choose> 
