									...
									<div class="controls">
										<select name="speciesId"  class="span12 m-wrap" tabindex="1">
											...
											<c:forEach var="species" items="${beans}"> 
										       <option <c:if test="${bean.speciesId eq species.speciesId}" >selected="selected"</c:if> value="${species.speciesId}">${species.speciesName}(${species.parentName})</option>
										　　</c:forEach>											
										</select>
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<div class="input-append date date-picker" data-date="2017-08-01" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
											<input style="height:34px" name="birthday" class="span9 m-wrap m-ctrl-medium date-picker" size="16" type="text" value="${bean.birthday}" /><span class="add-on"><i class="icon-calendar"></i></span>
										</div>
									</div>
								</div>
							</div>
						...
						<div class="row-fluid">
						   <div class="span6 ">
								<div class="control-group">
									...
									<div class="controls">
										<label class="radio">
										 <div class="radio hover"><span><input type="radio" <c:if test="${bean.gender == 1}">checked</c:if>  name="active" value="1"></span></div>男
										</label>
										<label class="radio">
										 <div class="radio hover"><span><input type="radio"<c:if test="${bean.gender == 0}">checked</c:if> name="active" value="0"></span></div>女
										</label>
									</div>
								</div>
							</div> 
