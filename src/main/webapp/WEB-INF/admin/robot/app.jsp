			...
			<!-- 应用管理 -->
			 <div class="portlet box" style="background-color: #eee;">
				 <div class="portlet-title" >
					<div class="caption" style="color:#3d3d3d!important"><i class="icon-reorder"></i>应用管理</div>
				 </div>
				<div class="portlet-body ">
					
					<c:forEach var="app" items="${beans}"  varStatus="status"> 
					   <c:if test="${status.index%4==0}"><div class="row-fluid margin-bottom-40"></c:if>
						<div class="span3 pricing hover-effect">
							<div class="pricing-head">
								<h3><nobr>${app.robotName}(${app.sceneName})</nobr><span><nobr>${app.sceneComment}</nobr></span></h3>
							</div>
							<ul class="pricing-content unstyled">
								<li><i class="icon-tags"></i>${app.application}</li>
								<li><i class="icon-asterisk"></i>${app.semantic}</li>
								<li><i class="icon-heart"></i> 逻辑数量</li>
								<li><i class="icon-shopping-cart"></i>受欢迎程度</li>
							</ul>
							<div class="pricing-footer">
								<c:choose>
								   <c:when test="${app.robotId eq bean.robotId}">  
								         <span onclick="rdswitch(this,'${bean.robotId}','${app.robotSceneId}')" class="btn grey">
										 关闭<i class="m-icon-swapright m-icon-white"></i>
										</span>
								   </c:when> 
								   <c:otherwise> 
		                                <span onclick="rdswitch(this,'${bean.robotId}','${app.robotSceneId}')" class="btn green">
...
<script>
	function rdswitch(obj,robotId,robotSceneId){
		var status = 1;
	    ...
	    formData.append("robotSceneId", robotSceneId);
	    formData.append("status", status);
	    $.ajax({  
	         url: '${basePath}/admin/robot/appswitch' ,  
	         type: 'POST',  
	         data: formData,  
	         contentType: false,  
	         processData: false,  
