		...
		<div class="span12">
			...
			<ul class="breadcrumb">
				...
				<li class="pull-right no-text-shadow">
					...
					<a href="${basePath}/admin/repository/addRe">添加知识库</a> 
					<i class="icon-angle-down"></i>
					<i class="icon-calendar"></i>
					<a href="${basePath}/admin/repository/addRiddle">添加谜语</a> 
					<i class="icon-angle-down"></i>
				</li>
	...
	<div class="row-fluid">
		<div class="portlet-body" style="display: block;">
			<table class="table table-striped table-bordered table-advance table-hover">
				<thead>
					<tr>
						<th><i class="icon-briefcase"></i>知识库ID</th>
						<th><i class="icon-briefcase"></i>雇主</th>
						<th><i class="icon-bookmark"></i>场景</th>
				...
				<tbody>
					<c:forEach var="repository" items="${beans}"> 
					    <tr>
							<td><a href="#">${repository.knowledgeId}</a></td>
							<td><a href="#">${repository.account}</a></td>
							<td>${repository.application}</td>
            ...
            <div class="pagination pagination-right">
				<ul>
					<li class=""><a href="${basePath}/admin/repository/list?curpage=1">首页</a></li> 
				<c:choose>
				   <c:when test="${curpage== '1'}">  
				    <li><a href="${basePath}/admin/repository/list?curpage=1">Prev</a></li>     
				   </c:when>
				   <c:otherwise> 
				   <li><a href="${basePath}/admin/repository/list?curpage=${curpage-1}">Prev</a></li>
				   </c:otherwise>
				</c:choose>
				<c:choose>
				    <c:when test="${curpage+2>5}">
						<c:forEach begin="1" end="${curpage-2>=5?5:curpage-3}" var = "page">
							<li class="${curpage==page?'active':''}"><a href="${basePath}/admin/repository/list?curpage=${page}">${page}</a></li> 
						</c:forEach>
					</c:when>
				</c:choose>
				<c:choose>
				...
				<c:choose>
				   <c:when test="${curpage== talpage}">  
				      <li><a href="${basePath}/admin/repository/list?curpage=${talpage}">Next</a></li>     
				   </c:when>
				   <c:otherwise> 
				      <li><a href="${basePath}/admin/repository/list?curpage=${curpage+1}">Next</a></li>
				   </c:otherwise>
				</c:choose>
					<li class=""><a href="${basePath}/admin/repository/list?curpage=${talpage}">尾页</a></li> 
				</ul>
			</div>
		</div>
	</div>
